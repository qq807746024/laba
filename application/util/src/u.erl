...
%% 检测
%u:c(5, "../application/gate/ebin/").
%u:c(5, "lib/gate-1/ebin/").
c(N) when is_number(N) ->
	Path = "../application/gate/ebin/",
	c(N, Path);
c(_) -> 
	info("ERROR======> Badarg", []).
...
c(M, Path) when is_integer(M) ->
    case file:list_dir(Path) of
        {ok, FileList} ->
			...
			info("---------check modules---------~p ~p", [length(Files), Files]);
        Any -> 
			info("Error Dir: ~w", [Any])
    end.

%% 更新
%u:u(5, "../application/gate/ebin/").
%u:u(5, "../application/network_proto/ebin/").
%u:u(5, "lib/gate-1/ebin/").
%u:u(5, "lib/db-1/ebin/").
...
u(N, Path) when is_number(N) ->
	case file:list_dir(Path) of
		{ok, FileList} -> 
			Files = get_new_file(FileList, util_math:ceiling(N * 60) + 3, Path),
