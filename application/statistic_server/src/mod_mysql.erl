...
-export([get_esql_select/3,get_esql_insert/3,get_esql_update/3,get_esql_delete/2,get_esql_replace/3, do_sql/2]).
...
-define(ERLYDB_OPTIONS,[{pool_id, ?MYSQL_CONNECT_POOL_ID},
						...
						{outdir, "../ebin/"}]).
-define(DEFAULT_TIMEOUT,10000). %%默认超时是5sec，这里修改为10sec
...
db_error_log(Module, Line, Level, FormatFun) when (Level=:=error) orelse (Level=:=warn)->
	...
	?ERROR_LOG("~w:~b: "++ Format ++ "~n", [Module, Line] ++ Arguments);
