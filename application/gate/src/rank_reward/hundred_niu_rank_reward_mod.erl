-module(hundred_niu_rank_reward_mod).
...
-define(HUNDRED_NIU_RANK_REWARD_MOD_STATE_CACHE_KEY, hundred_niu_rank_reward_mod_state_cache_key).
...
is_time_to_clean_rank() ->
    case game_earn_gold_rank_detail_config_db:get(hundred_niu) of
        {ok, [DetailConfig]} ->
...
get_start_and_end_days() ->
    case ets:lookup(?ETS_RANK_MOD_TIME_CACHE, ?HUNDRED_NIU_RANK_REWARD_MOD_STATE_CACHE_KEY) of
        [Ets] ->
            #ets_data {
                value = State
            } = Ets,
...
init_ets_from_db(Rank) ->
    case game_earn_gold_rank_reward_config_db:get_base({hundred_niu, Rank}) of
        {ok, [Config]} ->
            ets:insert(?ETS_HUNDRED_NIU_EARN_GOLD_RANK_REWARD_CONFIG, Config),
...
do_cast({'earn_gold_rank_reward_check', RankList, PoolDivide}, _State) ->
    ConfigList = ets:tab2list(?ETS_HUNDRED_NIU_EARN_GOLD_RANK_REWARD_CONFIG),
    case game_earn_gold_rank_detail_config_db:get(hundred_niu) of
        {ok, [DetailConfig]} ->
            #game_earn_gold_rank_detail_config {
                period = PeriodDays,
                            ...
                            RewardRankList = lists:foldl(
                                fun(E, Acc) ->
                                    ...
                                    if 
                                        ...
                                        true ->
                                            GoldNum =
                                                case lists:keyfind({hundred_niu, E#pb_rank_info.rank},#game_earn_gold_rank_reward_config.key,ConfigList) of
                                                    false ->
                                                        0;
                                                    Tuple ->
                                            ...
                                            player_mail:send_system_mail(EId, 1, ?MAIL_TYPE_HUNDRED_NIU_RANK_REWARD, Title, Content, [{?ITEM_ID_GOLD, GoldNum}]),
                                            [E#pb_rank_info{
                                                gold_num = GoldNum
                                            } | Acc]
                                    end
...
calc_days() ->
    case daily_rank_reward_history_db:get(hundred_niu) of
        {ok, [OldData]} ->
...
is_time_to_check_rank(PeriodDays, SpecialTimeOfDay) ->
    ...
    ?INFO_LOG("hundred_niu_gold_rank_reward_check --> ~p~n", [{DiffDays, PeriodDays}]),
    if
        DiffDays >= PeriodDays ->
            NowSeconds = util:now_seconds(),
            NotifySecond = util:hour_to_second(SpecialTimeOfDay),
            ?INFO_LOG("hundred_niu_gold_rank_reward_check --> ~p~n", [{NowSeconds, NotifySecond}]),
...
update_state()->
    State = case game_earn_gold_rank_detail_config_db:get(hundred_niu) of
        {ok, [DetailConfig]} ->
            #game_earn_gold_rank_detail_config {
                period = PeriodDays,
                special_time_of_day = SpecialTimeOfDay
        ...
        _ ->
            ?ERROR_LOG("not found any config for hundred_niu_rank_reward_mod.~n"),
            #state {
    ...
    ets:insert(?ETS_RANK_MOD_TIME_CACHE, #ets_data{
        key = ?HUNDRED_NIU_RANK_REWARD_MOD_STATE_CACHE_KEY, value = State
    }),
    State.