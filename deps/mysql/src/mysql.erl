...
start1(PoolId, Host, Port, User, Password, Database, LogFun, Encoding,
       StartFunc) ->
    crypto:start(),
    StartOptions = [PoolId, Host, Port, User, Password, Database, LogFun, Encoding],
    {ok,_} = supervisor:start_child(common_mysql_sup, 
                           {?SERVER, 
                            {?MODULE, ming2_start_link,[StartFunc,StartOptions]},
                            transient, brutal_kill, worker, [?MODULE]}).

ming2_start_link(StartFunc,StartOptions) ->
    [PoolId, Host, Port, User, Password, Database, LogFun, Encoding] = StartOptions,
...
get_pool_size()->
    call_server({get_pool_size}, undefined).
...
handle_call({get_pool_size}, _From, State) ->
    try
        #state{conn_pools=ConnPools} = State,
        [Vals] = gb_trees:values(ConnPools),
        if
            is_tuple(Vals)->
                PoolSize = length( lists:concat(tuple_to_list(Vals)) );
            is_list(Vals)->
                PoolSize = length( lists:concat(Vals) );
            true->
                PoolSize = -1
        end,
        {reply, {ok, PoolSize}, State}
    catch
        _:Err->
